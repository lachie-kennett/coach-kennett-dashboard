        ...
        <>
          {calorieData.length === 0 && macroData.length === 0 && qualData.length === 0 ? (
            <div className="text-center py-16 text-slate-500">
              ...
              <p className="text-sm mt-1">Fill in calories and macros in the daily tracker.</p>
            </div>
          ) : (
            <>
                  ...
                  <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-4">
                    ...
                    {calorieGoal && (
                      <div className="flex items-center gap-2 mt-2">
                        <div className="w-6 h-0.5 border-t-2 border-dashed border-orange-500 opacity-50" />
                        <span className="text-xs text-slate-500">Goal: {calorieGoal} kcal</span>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-4">
                    <ResponsiveContainer width="100%" height={100}>
                      <BarChart data={qualData} barSize={8}>
                        ...
                        <Tooltip
                          contentStyle={{ background: '#1e293b', border: 'none', borderRadius: 8, color: '#f1f5f9', fontSize: 12 }}
                          formatter={(_, __, props) => [props.payload.label, 'Quality']}
                        />
        ...
        <>
          {mealPlan.length === 0 ? (
            ...
            <>
              ...
              {!selectedDay && (
                <p className="text-slate-500 text-sm text-center py-8">Select a day above to view meals</p>
              )}
              ...
              {selectedDay && (() => {
                ...
                return (
                  <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-4 space-y-5">
                    {day.planName && (
                      <p className="text-xs text-orange-400 font-medium uppercase tracking-wide">{day.planName}</p>
                    )}
                    ...
                    <button
                      ...
                      className={`w-full flex items-start gap-3 rounded-2xl p-4 border text-left transition-all ${
                        isChecked
                          ...
                          : 'bg-slate-800/60 border-slate-700/50 hover:border-orange-500/30'
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`font-medium text-sm ${isChecked ? 'line-through text-slate-500' : 'text-white'}`}>
                          {item.name}
                        </p>
                        {item.notes && (
